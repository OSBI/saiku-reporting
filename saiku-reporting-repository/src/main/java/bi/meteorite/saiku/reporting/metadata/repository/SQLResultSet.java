...
public class SQLResultSet implements IPentahoResultSet, IPeekable {
  ...
  private static final Logger log = LoggerFactory.getLogger( SQLResultSet.class );
  ...
  public IPentahoMetaData getMetaData() {
    if ( metadata == null ) {
      try {
        ...
        SQLResultSet.log.error( "Exception message", e ); //$NON-NLS-1$
        // log.error(null, e);
  ...
  public Object[] next() {
    ...
    try {
      ...
      SQLResultSet.log.error( "Exception Message", e ); //$NON-NLS-1$
      try {
        throw new SQLResultSetException( "Exception Message", e ); //$NON-NLS-1$
      } catch (SQLResultSetException e1) {
        e1.printStackTrace();
      }
    }
    return null;
  ...
  public void close() {
    if ( nativeResultSet != null ) {
      try {
        ...
        SQLResultSet.log.warn( "Exception Message" ); //$NON-NLS-1$
      }
  ...
  public int getColumnCount() {
    ...
    try {
      ...
      SQLResultSet.log.error( "Exception Message", ex ); //$NON-NLS-1$
    }
    return 0;
  }
  ...
  public int getRowCount() {
    ...
    try {
      ...
      SQLResultSet.log.error(
          "Exception Message", sqle ); //$NON-NLS-1$
      rowCount = 0;
    }
    return rowCount;
  }
  ...
  public Object getValueAt( final int row, final int column ) {
    if ( nativeResultSet != null ) {
      try {
        ...
        SQLResultSet.log.error( "Exception Message", ex ); //$NON-NLS-1$
        throw new IllegalStateException( "Exception Message", ex ); //$NON-NLS-1$
      }
    }
    return null;
  }
  ...
  public void beforeFirst() {
    try {
      if ( nativeResultSet == null ) {
        SQLResultSet.log.error("Exception MEssage" ); //$NON-NLS-1$
      } else if ( isScrollable() ) {
        nativeResultSet.beforeFirst();
      } else {
        SQLResultSet.log.info( "Exception Message" );
      }
    } catch ( SQLException e ) {
      SQLResultSet.log.error( "Exception Message", e ); //$NON-NLS-1$
    }
